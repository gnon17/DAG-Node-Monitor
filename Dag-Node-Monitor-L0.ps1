#Change Log:
# 9/18/23: Added timer and timeout for 6 hours to the while loop to prevent infinite loop. Added additional check for while loop to detect status change and alert on changes between node state until node gets to Ready
...
$MyNode = try { invoke-webrequest https://nebula-apim.azure-api.net/public/dag/nodestate/integrationnet/"$NodeIP"?layer=l0 -UseBasicParsing | Select -ExpandProperty Content | Out-String -ErrorAction SilentlyContinue } catch { $_.Exception.Response }
